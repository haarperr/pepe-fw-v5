...
AddEventHandler('pepe-vehiclekeys:client:toggle:engine', function()
 ...
 Framework.Functions.TriggerCallback("pepe-vehiclekeys:server:has:keys", function(HasKey)
     if HasKey then
         ...
         Framework.Functions.Notify("Bạn không có chìa khóa của phương tiện này.", 'error')
     end
 end, Plate)
end)
    ...
    Framework.Functions.TriggerCallback("pepe-vehiclekeys:server:has:keys", function(HasKey)
        if HasKey then
            if Player ~= -1 and Player ~= 0 and Distance < 2.3 then
                ...
                Framework.Functions.Notify("Không có ai ở gần đó?", 'error')
            end
        else
            Framework.Functions.Notify("Bạn không có chìa khoá của phương tiện này.", 'error')
        end
    end, Plate)
end)
        ...
        Framework.Functions.TriggerCallback("pepe-vehiclekeys:server:has:keys", function(HasKey)
         if HasKey then
            ...
            if VehicleLocks == 1 then
                ...
                ClearPedTasks(PlayerPedId())
                TriggerEvent('pepe-vehicleley:client:blink:lights', Vehicle)
                Framework.Functions.Notify("Đã mở phương tiện!", 'success')
                TriggerServerEvent("pepe-sound:server:play:distance", 5, "car-unlock", 0.2)
            end
         else
            Framework.Functions.Notify("Bạn không có chìa khoá của phương tiện này.", 'error')
