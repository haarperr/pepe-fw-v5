    ...
    if not NitrousActivated then
        if IsInVehicle and not IsThisModelABike(GetEntityModel(GetVehiclePedIsIn(ped))) then
            ...
            if VehicleNitrous[Plate] == nil or VehicleNitrous[Plate].hasnitro == false then
                Framework.Functions.Progressbar("use_nos", "Using nitrous.", 1000, false, true, {
                    disableMovement = false,
                    disableCarMovement = false,
                    disableMouse = false,
                    disableCombat = true,
                }, {}, {}, {}, function() -- Done
                    TriggerEvent("pepe-inventory:client:ItemBox", Framework.Shared.Items['nitrous'], "remove")
            ...
            Framework.Functions.Notify('You are not in a car.', 'error')
        end
    end
end)
