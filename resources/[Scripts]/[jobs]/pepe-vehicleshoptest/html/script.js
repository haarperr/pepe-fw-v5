...
function menuVehicle(event) {
    ...
    $.post('https://pepe-vehicleshoptest/menuSelected', JSON.stringify({menuId: categoryVehicleSelected.toLowerCase()}));
}

function testDrive(){
    $.post('https://pepe-vehicleshoptest/TestDrive', JSON.stringify({vehicleModel: vehicleSelected.modelcar}));    
...
function buyVehicle(option) {
    ...
    switch(option){
        case 'cancel':
            break;
        case 'confirm':
            $.post('https://pepe-vehicleshoptest/Buy', JSON.stringify(vehicleSelected));
            break;
    }   
}
...
$(document).on('keydown', function() {
    switch(event.keyCode) {
        case 27: // ESC
            $.post('https://pepe-vehicleshoptest/Close');
            $('body').css("display","none");
        ...
        case 17: // TAB
            break;
        case 68: // LEFT A
            $.post('https://pepe-vehicleshoptest/rotate', JSON.stringify({key: "left"}))
            break;            
        case 65: // RIGHT D
            $.post('https://pepe-vehicleshoptest/rotate', JSON.stringify({key: "right"}))
            break;
        case 39: 
...
$(document).on('keydown', function(ev) {
    ...
    if (_key) {
        $.post('https://pepe-vehicleshoptest/rotate', JSON.stringify({key: _key}))
    }
    ...
    switch(event.which) {        
        ...
        case 1:
            ...
            if(!itemDisabled) {
                ...
                $.post("https://pepe-vehicleshoptest/SpawnVehicle", JSON.stringify({ modelcar: dataCar[9].outerText, price: dataCar[7].outerText }));
...
function setVehicleColorRGB(R,G,B){
    ...
    if (selectedColor == 'primary'){
        $.post("https://pepe-vehicleshoptest/RGBVehicle", JSON.stringify({primary: true, R: R, G: G, B: B}));
    } else {
        $.post("https://pepe-vehicleshoptest/RGBVehicle", JSON.stringify({primary: false, R: R, G: G, B: B}));
    }
}
