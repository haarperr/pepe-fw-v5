...
RegisterNetEvent('pepe-vehicleshoptest.receiveInfo')
AddEventHandler('pepe-vehicleshoptest.receiveInfo', function(bank, name)
    if name then
        profileName = name
    end
    profileMoney = bank
...
RegisterNetEvent('pepe-vehicleshoptest.successfulbuy')
AddEventHandler('pepe-vehicleshoptest.successfulbuy', function(vehicleName,vehiclePlate,value)    
    SendNUIMessage(
        {
            type = "successful-buy",
...
RegisterNetEvent('pepe-vehicleshoptest.notify')
AddEventHandler('pepe-vehicleshoptest.notify', function(type, message)    
    SendNUIMessage(
        {
            type = "notify",
            typenotify = type,
...
RegisterNetEvent('pepe-vehicleshoptest.vehiclesInfos')
AddEventHandler('pepe-vehicleshoptest.vehiclesInfos', function() 
...
function OpenVehicleShop()
    ...
    TriggerServerEvent("pepe-vehicleshoptest.requestInfo")
...
RegisterNUICallback(
    ...
    function(data, cb)
        ...
        TriggerServerEvent('pepe-vehicleshoptest.CheckMoneyForVeh',data.modelcar, data.sale, data.name, vehicleProps)
...
RegisterNetEvent('pepe-vehicleshoptest.spawnVehicle')
AddEventHandler('pepe-vehicleshoptest.spawnVehicle', function(model, plate)    
    local hash = GetHashKey(model)
    ...
    exports['pepe-fuel']:SetFuelLevel(vehicleBuy, GetVehicleNumberPlateText(vehicleBuy), 100, false)
    SetPedIntoVehicle(PlayerPedId(), vehicleBuy, -1)
    SetVehicleNumberPlateText(vehicleBuy, plate)
    exports['pepe-vehiclekeys']:SetVehicleKey(GetVehicleNumberPlateText(vehicleBuy), true)
...
RegisterNUICallback(
    ...
    function(data, cb)        
        if Config.TestDrive then
            ...
            exports['pepe-vehiclekeys']:SetVehicleKey(GetVehicleNumberPlateText(testDriveEntity), true)
