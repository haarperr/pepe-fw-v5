...
Citizen.CreateThread(function()
	...
	TriggerServerEvent("pepe-music:GetDate")
end)
...
RegisterNUICallback("action", function(data)
	...
	elseif data.action == "play" then
		if xSound:soundExists(nameid) then
			if xSound:isPaused(nameid) then
				TriggerServerEvent("pepe-music:ChangeState", true, nameid)
				local esperar = 0
				while nuiaberto do
					Wait(1000)
	...
	elseif data.action == "pause" then
		if xSound:soundExists(nameid) then
			if xSound:isPlaying(nameid) then
				TriggerServerEvent("pepe-music:ChangeState", false, nameid)
			end
		end
	...
	elseif data.action == "loop" then
		if xSound:soundExists(nameid) then
			...
			TriggerServerEvent("pepe-music:ChangeLoop",nameid,datasoundinfo.loop)
		else
	...
	elseif data.action == "forward" then
		if xSound:soundExists(nameid) then
			TriggerServerEvent("pepe-music:ChangePosition", 10, nameid)
		end
...
function ApplySound(quanti,plate)
	...
	if vadi <= 1.01 and vadi >= -0.001 and exis then
		...
		TriggerServerEvent("pepe-music:ChangeVolume", quanti, plate)
	end
end
...
function SetUrl(url,nid)
	...
	if url then
		...
		if encontrad then
			...
			TriggerServerEvent("pepe-music:ModifyURL",vehdata)
		else
			if IsPedInAnyVehicle(PlayerPedId(), false) then
				local veh = GetVehiclePedIsIn(PlayerPedId(),false)
				...
				vehdata.loop = datasoundinfo.loop
				TriggerServerEvent("pepe-music:AddVehicle",vehdata)
			end
		end
	else
...
if Config.ItemInVehicle then
	RegisterNetEvent("pepe-music:ShowNui")
	AddEventHandler("pepe-music:ShowNui", function()
		show()
	end)
end
...
AddEventHandler("pepe-music:AddVehicle", function(data)
...
AddEventHandler("pepe-music:ChangeState", function(tipo, nome)
...
AddEventHandler("pepe-music:ChangeLoop", function(tipo, nome)
	if xSound:soundExists(nome) then
...
AddEventHandler("pepe-music:ChangeVolume", function(som, range, nome)
	local carroe
	local crds
    for i = 1, #Zones do
        local v = Zones[i]
...
RegisterNetEvent("pepe-music:SendData")
AddEventHandler("pepe-music:SendData", function(data)
    Zones = data
    for i = 1, #Zones do
		local v = Zones[i]
		if v.isplaying then
