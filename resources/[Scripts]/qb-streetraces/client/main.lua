...
AddEventHandler('qb-streetraces:RaceDone', function(race, winner)
    if RaceId ~= 0 and RaceId == race then
        RaceId = 0
        InRace = false
        Framework.Functions.Notify("Race ended! The winner is "..winner.. "!")
    end
end)
...
AddEventHandler('qb-streetraces:CreateRace', function(amount)
    ...
    if DoesBlipExist(WaypointHandle) then
        ...
        if GetDistanceBetweenCoords(pos.x, pos.y, pos.z, cx, cy, groundZ, true) > 500.0 then
            ...
            Framework.Functions.Notify("Race made for $"..amount..",-!", "success")
        else
            Framework.Functions.Notify("End position too close!", "error")
        end
    else
        Framework.Functions.Notify("You need to drop a marker!", "error")
    end
end)
...
function RaceCountDown()
    ...
    while RaceCount ~= 0 do
        ...
        Framework.Functions.Notify(RaceCount, 'primary', 800)
        Citizen.Wait(1000)
    ...
    Framework.Functions.Notify("GOOOOOOOOO!!!")
end
