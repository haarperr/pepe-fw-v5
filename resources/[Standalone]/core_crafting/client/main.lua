...
Citizen.CreateThread(function()
    ...
    for _, v in pairs(Framework.Shared.Items) do
        ...
        grade = Framework.Functions.GetPlayerData().job.grade
...
RegisterNetEvent("Framework:Client:OnJobUptade")
AddEventHandler(
    "Framework:Client:OnJobUptade",
    function(j)
        job = j.name
        grade = j.grade
    end
...
function openWorkbench(val)
    Framework.Functions.TriggerCallback(
        ...
        function(xp)
            ...
            local inv = {}
            for _, v in ipairs(Framework.Functions.GetPlayerData().inventory) do
                inv[v.name] = v.count
            end
